...
const fetchResponse = async (input) => {
  ...
  try {
      ...
      messages: [
        ...
        { role: "system", content: `You are my personal assistant` },
        {
          ...
          content: [
            {
              type: "text",
              text: "what is in the image and what is he thinking ?",
            },
            {
              type: "image_url",
              image_url:
                "https://images.unsplash.com/photo-1607337202714-a88f7abbdee7?q=80&w=1770&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
            },
          ],
        },
      ],
    ...
    const length = chats.value.length;
    inputFreeze.value = false;
    // adding to stack
    chats.value.push({ role: "system", content: "" });
    for await (const chunk of completion) {
      console.log(chunk.choices[0].delta.content);
      if (chunk.choices[0].delta.content) {
        chats.value[length] = {
          role: "system",
          content: chats.value[length].content + chunk.choices[0].delta.content,
        };
      }
      console.log(length);
    }
    // start speech to txt
    const utterance = new SpeechSynthesisUtterance(chats.value[length].content);
